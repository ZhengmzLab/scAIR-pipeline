...
then 
    ...
    ${pipe_dir}/Tools/java -jar ${pipe_dir}/Tools/juicer_tools_1.19.01.jar pre -r 2500000,1000000,500000,250000,100000,50000,25000,10000,5000,1000 ${name}.ChIADrop.SubGEM.Clean.sorted.juice ${name}.hic ${tgenome}
    if [[ ! -d StatFiles ]]
    then
        mkdir StatFiles
    fi
...
then
    ...
    zcat ${name}.Pairs.gz | awk '{if($0 ~ /^#/) {print} else {if(($9 == "pair" || $9=="self") && $15 != "dup") print}}' | ${pipe_dir}/Tools/pigz > ${name}.Dedup.Frags.gz
    zcat ${name}.Dedup.Frags.gz | awk '{if($0 ~ /^#/) {print} else {if($9 == "pair") print}}' | ${pipe_dir}/Tools/pigz > ${name}.Dedup.Pairs.gz
    ...
    ${pipe_dir}/Tools/java -jar ${pipe_dir}/Tools/juicer_tools_1.19.01.jar pre ${name}.Dedup.Pairs.Sorted.gz ${name}.Dedup.Pairs.hic ${genomesize}
