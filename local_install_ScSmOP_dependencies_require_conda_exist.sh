...
conda install bedtools -n ScSmOP -y 
ln -s ${ScSmOPCondaDir}/bin/bedtools bedtools

${ScSmOPCondaDir}/bin/pip install macs2
...
conda install openjdk=8 -n ScSmOP -y
ln -s ${ScSmOPCondaDir}/bin/java java
...
# check installation 
InstallSuccesss=1
for tool in samtools macs2 bedtools Rscript bwa java pairtools
do
    if [[ ! -f Tools/${tool} ]]
    then
        InstallSuccesss=0
